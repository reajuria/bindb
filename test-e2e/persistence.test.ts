...
describe('Persistence', () => {
  ...
  it('data persistence across server restarts', async () => {
  ...
  it('schema persistence and validation', async () => {
    ...
    try {
      ...
      expect(persistedSchema.columns.length).toBeGreaterThanOrEqual(originalSchema.length);
  ...
  it('large dataset persistence', async () => {
    ...
    try {
      ...
      for (const id of sampleIds) {
        ...
        expect(findResponse.status).toBe(200);
        expect(findResponse.data.found).toBe(true);
        expect(findResponse.data.record?.data.includes('Test data record')).toBeTruthy();
      }
