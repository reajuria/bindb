...
test('type safety with different value types', () => {
  ...
  assert.deepEqual(retrievedUser, user);
});

test('cache with complex key types', () => {
  // Test with number keys
  const numKeyCache = new LRUCache<number, string>(3);
  numKeyCache.set(1, 'one');
  numKeyCache.set(2, 'two');
  
  assert.equal(numKeyCache.get(1), 'one');
  assert.equal(numKeyCache.get(2), 'two');
  assert.equal(numKeyCache.has(1), true);
  assert.equal(numKeyCache.has(3), false);
});